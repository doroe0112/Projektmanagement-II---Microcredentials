...
document.getElementById("bubble-chart").setAttribute("width", width);
document.getElementById("bubble-chart").setAttribute("height", height);
...
const generateChart = data => {
    ...
    const circle = node.append('circle')
        .style('fill', d => colors[d.data.semester])
...
(async () => {
    let data = await d3.json(file).then(data => data);
    generateChart(data);
})();